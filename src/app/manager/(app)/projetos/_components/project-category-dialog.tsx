...
export function ProjectCategoryDialog({ category, onSuccess }: Props) {
  const [open, setOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [color, setColor] = useState(() =>
    extractBgColorKey(category?.color ?? COLOR_OPTIONS[0].key)
  )
  ...
  function handleOpenChange(newOpen: boolean) {
    if (newOpen) {
      setColor(extractBgColorKey(category?.color ?? COLOR_OPTIONS[0].key))
    }
    setOpen(newOpen)
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const url = isEdit
      ? `/api/project-categories/${category.id}`

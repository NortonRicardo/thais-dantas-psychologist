...
export function ProjectCategoriesTable() {
  const [rows, setRows] = useState<ProjectCategoryRow[]>([])
  const [loading, setLoading] = useState(true)
  const [page, setPage] = useState(1)
  const [deleteTarget, setDeleteTarget] = useState<ProjectCategoryRow | null>(
    null
  )
  ...
  async function handleConfirmDelete() {
    ...
    try {
      const res = await fetch(`/api/project-categories/${deleteTarget.id}`, {
        method: 'DELETE',
      })
      if (!res.ok) {
        toast.error(await readApiError(res))
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-sm text-white/40">
            Tipos de projeto (TCC, plataforma, …) usados na listagem pública e
            no gestor.
          </p>
        </div>
        <div className="flex shrink-0 self-end sm:self-start sm:pt-0.5">
          <ProjectCategoryDialog onSuccess={fetchCategories} />
        </div>
      </div>
        ...
        <AlertDialogContent className="bg-[#071525] border-white/10 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/50">
              {deleteTarget ? (
                <>
                  &ldquo;{deleteTarget.title}&rdquo; será removida
                  permanentemente.
                </>
              ) : null}
            </AlertDialogDescription>

...
import { TeamPrefixDialog, type TeamPrefixRow } from './team-prefix-dialog'
...
export function TeamPrefixesTable() {
  const [rows, setRows] = useState<TeamPrefixRow[]>([])
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/team/prefixes')
  ...
  async function handleDelete(id: string) {
    try {
      const res = await fetch(`/api/team/prefixes/${id}`, { method: 'DELETE' })
      if (!res.ok) {
      ...
      toast.error(err instanceof Error ? err.message : 'Erro ao remover tratamento.')
    }
  }
    ...
    <div className="space-y-4">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
        <div className="min-w-0">
          <h1 className="text-xl font-semibold text-white/90">Tratamentos</h1>
          <p className="mt-0.5 text-sm text-white/40">
            Prefixos opcionais antes do nome (Dr., Prof. Dr., …) na equipe pública e nos projetos.
          </p>
        </div>
        <div className="flex shrink-0 self-end sm:self-start sm:pt-0.5">
          <TeamPrefixDialog onSuccess={fetchRows} />
        </div>
      </div>
                ...
                >
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <TeamPrefixDialog prefix={row} onSuccess={fetchRows} />
                      <AlertDialog>
                        ...
                        <AlertDialogContent className="bg-[#071525] border-white/10 text-white">
                          <AlertDialogHeader>
                            <AlertDialogTitle>Remover tratamento?</AlertDialogTitle>

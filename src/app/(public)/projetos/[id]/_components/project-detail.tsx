...
import { Github, ExternalLink, CalendarDays, Users, BookOpen, GraduationCap, FlaskConical, FileText } from 'lucide-react'
...
function MetaRow({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[0.58rem] uppercase tracking-[2px] text-white/30">{label}</span>
      <span className="flex items-start gap-1.5 text-xs text-white/70">
        <Icon size={11} strokeWidth={1.5} className="mt-0.5 shrink-0 text-white/35" />
        {value}
      </span>
    </div>
  )
  ...
  return (
    <div className="flex w-full flex-col gap-8 pb-16 lg:flex-row lg:items-start">
      ...
      <aside className="w-full shrink-0 lg:w-1/4">
        <div className="sticky top-6 flex flex-col gap-5 rounded-2xl px-5 py-6" style={glass}>
          ...
          {project.endDate ? (
            ...
            <div className="flex flex-col gap-0.5">
              <span className="text-[0.58rem] uppercase tracking-[2px] text-white/30">Status</span>
          ...
          {project.advisor && (
            <MetaRow icon={GraduationCap} label="Orientador(a)" value={project.advisor} />
          )}

          {project.coAdvisor && (
            <MetaRow icon={GraduationCap} label="Coorientador(a)" value={project.coAdvisor} />
          )}

          {project.researchLead && (
            <MetaRow icon={FlaskConical} label="Responsável pela pesquisa" value={project.researchLead} />
          )}
      ...
      <main className="flex min-w-0 flex-1 flex-col gap-8">
        ...
        <div className="flex flex-col gap-3">
          ...
          {project.pdfPath ? (
            <div className="overflow-hidden rounded-2xl border border-white/15" style={{ height: '70vh' }}>
              <iframe
                src={project.pdfPath}
            ...
            >
              ...
              <p className="text-xs text-white/25">O documento será disponibilizado após a conclusão do trabalho.</p>
            </div>
          )}
        </div>
      </main>
